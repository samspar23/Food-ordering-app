...
@NamedQueries({
        @NamedQuery(name = "restaurantsByName", query = "select r from RestaurantEntity r where UPPER" +
                "(r.restaurantName) LIKE concat('%',UPPER(:restaurantName),'%') ORDER BY r.restaurantName ASC"),
        ...
        @NamedQuery(name = "restaurantByCategory", query = "select distinct r FROM RestaurantEntity r " +
                "INNER JOIN r.categories c WHERE c.uuid = :uuid " +
